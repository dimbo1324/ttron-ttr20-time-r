...
.PHONY: help fmt test test-race test-fuzz build build-client build-emulator build-gateway build-cli build-api run-emulator run-client run-gateway run-api proto check-architecture web-install web-dev web-build web-typecheck web-lint verify-web verify clean
