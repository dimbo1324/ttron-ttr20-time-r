...
func (s *Service) buildReadTimeResponse(req frame.Frame) ([]byte, error) {
	return s.wire.EncodeReadTimeResponse(req, s.clock.Now())
}

func (s *Service) buildReadIdentityResponse(req frame.Frame) ([]byte, error) {
	return s.wire.EncodeReadIdentityResponse(req, s.Identity())
}
